...
def test_serialization_consistency(TestBaseConfig, TestDerived, OtherTestDerived):
    to_serialize: List[TestBaseConfig] = [
        TestDerived.TestDerivedConfig("Hello", 42),
        OtherTestDerived.OtherTestDerivedConfig("World", 3.14),
    ]
...
def test_configurable_adds_config(TestDerived):
    assert isinstance(TestDerived.TestDerivedConfig, Config)
...
def test_config_fields(TestDerived):
    assert {
        "base_prop": str,
        "derived_field": int,
    } == TestDerived.TestDerivedConfig.__annotations__


def test_config_inherits(TestDerived, TestBaseConfig):
    assert TestBaseConfig in TestDerived.TestDerivedConfig.__bases__


def test_config_configures(TestDerived):
    assert TestDerived == TestDerived.TestDerivedConfig.configures()
